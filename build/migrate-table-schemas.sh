...
ALTER SEQUENCE teacher_id_seq RESTART WITH 2000;

CREATE TABLE IF NOT EXISTS section (
id serial not null primary key,
code char(1) unique not null);
ALTER SEQUENCE teacher_id_seq RESTART WITH 3000;

CREATE TABLE IF NOT EXISTS course (
id serial not null primary key,
code char(4) unique not null);
ALTER SEQUENCE teacher_id_seq RESTART WITH 4000;
...
ALTER SEQUENCE teacher_id_seq RESTART WITH 5000;

CREATE TABLE IF NOT EXISTS class_teacher (
id serial not null primary key,
class_id integer not null references class(id),
teacher_id char(6) not null references teacher(code));
ALTER SEQUENCE teacher_id_seq RESTART WITH 6000;
