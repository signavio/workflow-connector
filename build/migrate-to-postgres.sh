...
ON_HEROKU=${DYNO:-false}
# Add database to connect to if we are running on heroku
if [ "${ON_HEROKU}" == "false" ]
then
    PSQL_COMMAND="PGPASSWORD=${PGPASSWORD} psql -a -U ${POSTGRES_ROOT_USER} -h ${POSTGRES_ROOT_HOST}"
else
    PSQL_COMMAND="PGPASSWORD=${PGPASSWORD} psql -a -U ${POSTGRES_ROOT_USER} -h ${POSTGRES_ROOT_HOST} ${POSTGRES_DATABASE}"
fi
cat << __EOF__ | eval "${PSQL_COMMAND}"
